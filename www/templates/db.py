    ...
    2.调用接口
        1.初始化数据库连接信息
            create_engine封装了如下功能：
                1.为数据库连接准备需要的配置信息
                2.创建数据连接（由生成的全局对象 engine的 connect方法提供）
...
def next_id(t=None):
    """
    生成一个唯一id，由当前时间+随机数（由伪随机数得来）拼接而成。
    """
    if t is None:
        t = time.time()
...
def create_engine(user, password, database, host='127.0.0.1', port=3306, **kw):
    """
    db模块的核心函数，用于连接数据库，生成全局对象 engine，
    engine对象持有数据库连接
    """
...
def connection():
    ...
    通过 _ConnectionCtx对_db_ctx封装，使得惰性连接可以自动获取和释放，
...
def select_one(sql, *args):
    ...
    >>> u1 = dirt(id=100, name='Alice', email='alice@test.org', passwd='ABC-12345', last_modified=time.time())
    >>> u2 = dirt(id=101, name='Sarah', email='sarah@test.org', passwd='ABC-12345', last_modified=time.time())
    >>> insert('user', **u1)
    1
    >>> insert('user', **u2)
    1
...
class _DbCtx(threading.local):
    """
    db模块的核心对象，数据库连接的上下文对象，负责从数据库获取和释放连接取得的
    连接是惰性连接对象，因此只有调用cursor对象时，才会真正获取数据库连接该对象
    是一个Thread local 对象，因此绑定在此对象上的数据仅对本线程可见
    """
    def __init__(self):
        self.connection = None
        self.transactions = 0
...
class _ConnectionCtx(object):
    """
    因为 _DbCtx实现了连接的获取和释放，但是并没有实现连接的自动获取
    和释放，_ConnectCtx 在 _DbCtx 的基础上实现了该功能，因此可以对
    _ConnectCtx 使用with语法，比如：
    with connection():
        pass
        with connection():
            pass
...
class _TransactionCtx(object):
    ...
    def __enter__(self):
        global _db_ctx
        self.shouldc_close_conn = False
        if not _db_ctx.is_init():
            # needs open a connection first
            _db_ctx.init()
            self.shouldc_close_conn = True
...
if __name__ == '__main__':
    ...
    create_engine('root', 'ybt360hxn', 'web_app', '127.0.0.1')
